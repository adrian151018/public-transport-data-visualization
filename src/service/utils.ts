...
export async function loadStopData(zipFile: JSZip){
    const stops = await loadStops(zipFile);
    if(!stops)throw new Error("Error while loading stops!");
    return stops.data;
}

export function getStopByCode(stops: Record <string, string>[], stopCode: string){
...
async function getServicesForToday(zipFile: JSZip, servicesFile: Papa.ParseResult<Record <string, string>> | undefined = undefined){
    ...
    if(!parsedServices)throw new Error("Error while loading services!");
...
async function getTripsForToday(
    zipFile: JSZip, tripsFile: Papa.ParseResult<Record <string, string>> | undefined = undefined, 
    servicesFile: Papa.ParseResult<Record <string, string>> | undefined = undefined){
    const parsedTrips = tripsFile ? tripsFile : await loadTrips(zipFile);
    if(!parsedTrips)throw new Error("Error while loading trips!");
...
async function getTripsByIds(
    zipFile: JSZip, 
    tripsFile: Papa.ParseResult<Record <string, string>> | undefined = undefined, 
    routesFile: Papa.ParseResult<Record <string, string>> | undefined = undefined, 
    tripIds: string[]){
        const parsedTrips = tripsFile ? tripsFile : await loadTrips(zipFile);
        if(!parsedTrips)throw new Error("Error while loading trips!");
        ...
        if(!parsedRoutes)throw new Error("Error while loading routes!");
        ...
        tripIds.forEach(trip => {
            ...
            if(!route)throw new Error("No route ID for trip ID " + trip + "!");
            const routeobj = routeMap.get(route);
            if(!routeobj)throw new Error("No such route!");
...
export function getStopSchedule(
    ...
    servicesFile: Papa.ParseResult<Record <string, string>> | undefined = undefined, 
    routesFile: Papa.ParseResult<Record <string, string>> | undefined = undefined, 
    stopTimesFile: Papa.ParseResult<Record <string, string>> | undefined = undefined
){
    return getTripsForToday(zipFile, tripsFile, servicesFile)
        .then(async tripsToday => {
            ...
            if(!parsedStopTimes)throw new Error("Error while loading stop schedule!");
            const tripsTodayIdSet = new Set();
            const todaysTripsToStop: string[] = []
            const filteredStopTimes: Record <string, string>[] = [];
            const schedule: Scheduled[] = [];                    
            tripsToday.forEach(trip => {
                tripsTodayIdSet.add(trip["trip_id"]);
            })

            for(const entry of parsedStopTimes.data){
                if(
            ...
            return getTripsByIds(zipFile, tripsFile, routesFile, todaysTripsToStop)
                .then(trips => {
                    for(const i in filteredStopTimes){
...
export async function getAllStopsSchedules(zipFile: JSZip){
    ...
    if(!parsedStopTimes)throw new Error("Error while parsing scheduled stop times!");
    const parsedTrips = await loadTrips(zipFile);
    if(!parsedTrips)throw new Error("Error while parsing trips!");
    const parsedRoutes = await loadRoutes(zipFile);
    if(!parsedRoutes)throw new Error("Error while parsing routes!");
    const tripsToday = await getTripsForToday(zipFile, parsedTrips);
    const stopMap: Map<Stop, Scheduled[]> = new Map();

    const parsedStops = await loadStops(zipFile);
    if(!parsedStops)throw new Error("Error while parsing stops!");
